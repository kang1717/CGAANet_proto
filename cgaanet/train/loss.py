...
class IntraGrainForceLoss(LossDefinition):
    """
    Loss for intra-grain atomic force
    """

    def __init__(
        self,
        name: str = 'IntraGrainForce',
        unit: str = 'eV/A',
        criterion: Optional[Callable] = None,
        ref_key: str = KEY.RELATIVE_FORCE,
        pred_key: str = KEY.INTRA_GRAIN_FORCES,
    ):
        super().__init__(
            name=name,
            unit=unit,
...
def get_loss_functions_from_config(config: Dict[str, Any]):
    from cgaanet.train.optim import loss_dict
    ...
    loss_functions.append((IntraGrainForceLoss(), config[KEY.INTRA_GRAIN_FORCE_WEIGHT]))
