...
def pt_to_args(pt_filename: str):
    ...
    Usage:
        dataset = SevenNetGraphDataset(
            **pt_to_args({path}/cgaanet_data/dataset.pt)
        )
    """
...
class SevenNetGraphDataset(InMemoryDataset):
    """
    Replacement of AtomGraphDataset. (and .cgaanet_data)
    ...
    f'{root}/cgaanet_data/{processed_name}.pt
    ...
    Args:
        ...
        files: list of filenames to initialize dataset:
               ASE readable (with proper extension), structure_list, .cgaanet_data
        process_num_cores: # of cpu cores to build graph
        processed_name: save as {root}/cgaanet_data/{processed_name}.pt
        ...
        force_reload: if True, reload dataset from files even if there exist
                      {root}/cgaanet_data/{processed_name}
    ...
    def __init__(
        ...
        **process_kwargs,
        ...
        self._processed_names = [
            processed_name,  # {root}/cgaanet_data/{name}.pt
            processed_name.replace('.pt', '.yaml'),
        ]

        root = root or './'
        _pdir = os.path.join(root, 'cgaanet_data')
        ...
        if not os.path.exists(_pt) and len(self._files) == 0:
            raise ValueError(
                (
                    ...
                    + 'whole cgaanet_data dir without changing its name.'
    ...
    def processed_dir(self) -> str:
        return os.path.join(self.root, 'cgaanet_data')
    ...
    @staticmethod
    def _read_cgaanet_data(filename: str) -> tuple[list[AtomGraphData], float]:
        ...
        if isinstance(dataset, AtomGraphDataset):
            ...
            raise ValueError(f'Not cgaanet_data type: {type(dataset)}')
    ...
    def file_to_graph_list(
        ...
        elif filename.endswith('.cgaanet_data'):
            graph_list, cutoff_other = SevenNetGraphDataset._read_cgaanet_data(
                filename
            )
...
def from_config(
    ...
    '''
    try:
        sample_graph = datasets['trainset'][0]
        if KEY.RELATIVE_POS in sample_graph:
            log.writeline(f"{KEY.RELATIVE_POS} field found in the dataset.")
        else:
            log.writeline(f"{KEY.RELATIVE_POS} field is missing in the dataset.")
    except Exception as e:
        log.writeline(f"Error accessing sample graph: {e}")
        raise
    '''
    ...
    log.writeline('Auto detect grain information')
    preprocess_grain_info(train_set, config)
