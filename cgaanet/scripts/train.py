...
from cgaanet.model_build import build_E3_equivariant_model
from cgaanet.cgaanet_logger import Logger
from cgaanet.train.trainer import Trainer

def print_model_parameters(model,log):
    """
    Print the number of parameters in each layer of the model.
    """
    total_params = 0
    log.write("=== Model Parameter Summary ===\n")
    for name, layer in model.named_children():
        layer_params = sum(p.numel() for p in layer.parameters() if p.requires_grad)
        log.write(f"Layer: {name}, Parameters: {layer_params}\n")
        total_params += layer_params

    log.write("===============================\n")
    log.write(f"Total Parameters: {total_params}\n")
...
def train_v2(config, working_dir: str):
    ...
    """
    import cgaanet.train.atoms_dataset as atoms_dataset
    import cgaanet.train.graph_dataset as graph_dataset
...
def train(config, working_dir: str):
    ...
    datasets['dataset'][0][KEY.BATCH] = tensor([1,1,1])
    data_out = model(datasets['dataset'][0])
    print("\nFinal NODE_FEATURE shape:", data_out[KEY.NODE_FEATURE].shape)

    ''' 

    log.write('Model building was successful\n')
    print_model_parameters(model,log)
