...
from cgaanet.error_recorder import ErrorRecorder
from cgaanet.cgaanet_logger import Logger
from cgaanet.train.trainer import Trainer
...
def make_parity_plot(outputs):

    with open("energy_parity","w") as ef, open("force_parity","w") as ff:

        for output in outputs:

            f_pred = output[KEY.INTRA_GRAIN_FORCES]
            f_orig = output[KEY.RELATIVE_FORCE]

            mask = (output[KEY.TRIPLE_ZERO] == 1)

            F_PRED = f_pred[mask] 
            F_ORIG = f_orig[mask]

            E_PRED = output[KEY.PRED_TOTAL_ENERGY].reshape(-1)/output[KEY.TOTAL_ATOMS].reshape(-1)
            E_ORIG  = output[KEY.ENERGY].reshape(-1)/output[KEY.TOTAL_ATOMS].reshape(-1)

            for x,y in zip(F_ORIG,F_PRED):

                ff.write(f"{x.item()} {y.item()}\n")

            for x,y in zip(E_ORIG,E_PRED):

                ef.write(f"{x.item()} {y.item()}\n")
...
def processing_epoch(trainer, config, loaders, start_epoch, init_csv, working_dir):
    ...
    for epoch in range(start_epoch, fin_epoch):
        ...
        valid_output = trainer.run_one_epoch(valid_loader, error_recorder=valid_recorder)
