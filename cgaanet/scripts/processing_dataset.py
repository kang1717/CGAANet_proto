...
from cgaanet.util import chemical_species_preprocess, onehot_to_chem
#from cgaanet.grain_read import preprocess_grain_info, revise_force_format
...
def dataset_load(file: str, config):
    ...
    graph prebuilt cgaanet_data
    """
    log = Logger()
    log.write(f'Loading {file}\n')
    log.timer_start('loading dataset')

    if file.endswith('.cgaanet_data'):
...
def processing_dataset(config, working_dir):
    ...
    preprocess_grain_info(config)

    #dataset = revise_force_format(dataset,config)

    #print (dataset.dataset['No_label'][0]['relative_forces'])
    ...
    if save_dataset:
        if save_dataset.endswith('.cgaanet_data') is False:
            save_dataset += '.cgaanet_data'
    ...
    if KEY.LOAD_VALIDSET in config and config[KEY.LOAD_VALIDSET]:
        ...
        if set(valid_labels).issubset(set(train_labels)) is False:
            ...
            log.write('Please create validset by cgaanet_graph_build with -l\n')
