...
_ERROR_TYPES = {
    ...
    'IntraGrainForce': {
        'name': 'IntraForce',
        'ref_key': KEY.RELATIVE_FORCE,
        'pred_key': KEY.INTRA_GRAIN_FORCES,
        'unit': 'eV/Å',
        'vdim': 3
    }, 
    'Stress': {
...
class ErrorRecorder:
    ...
    def init_total_loss_metric(config, criteria):
        ...
        intra_grain_force_metric = CustomError(criteria, **get_err_type('IntraGrainForce'))
        metrics.append((intra_grain_force_metric, config[KEY.INTRA_GRAIN_FORCE_WEIGHT]))
        if is_stress:
