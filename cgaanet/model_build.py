...
import cgaanet._const as _const
...
from .nn.force_output import ForceStressOutputFromEdge, ForceOutput
...
from cgaanet.nn.grain_embedding import GrainEncoding, IrrepsLinear_grain_embedding, IntraGrainActivation, save_additional_key, split_energy_force_line, make_average_zero, ForceOutputGrain, MergeForces,make_zero
...
def build_E3_equivariant_model(config: dict, parallel=False):
    ...
    grain_encoding_dim = sum(config["GRAIN_ATOM_COUNTS"].values())

    grain_encoding_irreps = Irreps(f'{grain_encoding_dim}x0e+{grain_encoding_dim}x1o')

    irreps_x = (
        Irreps(f'{feature_multiplicity}x0e+{feature_multiplicity}x1o')
    ...
    # encoding layer
    layers.update({
        'grain_encoding': GrainEncoding(
            grain_atom_counts=config["GRAIN_ATOM_COUNTS"],
            device=device,
            data_key_type=KEY.NODE_FEATURE,      
            data_key_relative_pos=KEY.RELATIVE_POS,
        ),
    })
     
    layers.update({
        'save_additional_key': save_additional_key()
    })

    # embedding layer
    layers.update({
        'intra_grain_linear_x1': IrrepsLinear(
                irreps_in=grain_encoding_irreps,
                irreps_out=irreps_x,
                data_key_in = KEY.NODE_FEATURE,
    ...
    '''
    layers.update(IntraGrainActivation(
        t=1,
        irreps_x= irreps_x,
        act_scalar_dict= act_scalar,
        act_gate_dict= act_gate,
        data_key_x = KEY.NODE_FEATURE,
        biases=use_bias_in_linear,
    ))
    layers.update({
        'intra_grain_linear_x2': IrrepsLinear(
                irreps_in=irreps_x,
                irreps_out=irreps_x,
                data_key_in =KEY.NODE_FEATURE,
                biases=use_bias_in_linear,
    ...
    layers.update({
        #'intra_grain_linear_x3': IrrepsLinear_grain_embedding(
        'intra_grain_linear_x3': IrrepsLinear(
                irreps_in=irreps_x,
                irreps_out=irreps_x,
                data_key_in =KEY.NODE_FEATURE,
                biases=use_bias_in_linear,
    ...
    for t in range(num_convolution_layer):
        ...
        layers.update(interaction_builder(grain_encoding_dim=grain_encoding_dim,feature_multiplicity=feature_multiplicity,**param_interaction_block))
        irreps_x = irreps_out


    #### SPLIT ENERGY AND "ATOMIC (INTRA GRAIN)" FORCE (NOT TOTAL GRAIN FORCE) LINES ####

    max_length = max(config["GRAIN_ATOM_COUNTS"].values())

    irreps_force_in  = Irreps(f"{feature_multiplicity}x1o")
    irreps_force_out = Irreps(f"{grain_encoding_dim}x1o")

    layers.update({
            'split_0e_1o': split_energy_force_line(irreps_x),
    })


    layers.update({
            'intra_grain_linear_force': IrrepsLinear(
                irreps_in=irreps_force_in,
                irreps_out=irreps_force_out,
                data_key_in = KEY.FORCE_TMP,
                data_key_out = KEY.INTRA_GRAIN_FORCES,
                biases=use_bias_in_linear,
            ),
            'make_zero': make_zero(
            grain_atom_counts=config["GRAIN_ATOM_COUNTS"],
    ...
    irreps_e = Irreps(f'{feature_multiplicity}x0e')

    layers.update(init_feature_reduce(config, irreps_e))
    ...
    layers.update({'merge_total_force': MergeForces()})
